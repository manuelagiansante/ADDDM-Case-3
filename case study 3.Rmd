...
In `lavaan` syntax, `=~` means "is measured by". What we are designing here is a measurement
model, because the latent variables predict the observed exogenous
variables.By default, `lavaan` does allows the factors ot correlate together. In order to
avoid that, it is possible to use the syntax `factor1~~0*factor2` to set a correlation between two factors to zero. However, this choice depends on the data we have, it is an option to be considered if our models have poor fits.
...
$$H_0 : \Sigma = S$$
If the $\chi^2$ test's null hypothesis is rejected, it indicates that the model might not fit well the structure underlying our data, as the reproduced correlation matrix is too different from the original one.
Here, as the $p$-value of the $\chi^2$ test is 0.00, we should reject the null hypothesis, which indicates that the model does not fit the structure of our data well.
...
$$RMSEA = \sqrt{\dfrac{\chi - df}{df(N-1)}}$$
where $\chi$ is the value of the $\chi^2$ test, $df$ is the number of degrees of freedom and $N$ is the sample size of the data, which is 553 in our case.
...
$$H_0: RMSEA \leq 0.05$$
In our case, the $p$-value of the test of good fit is 0.971, which means that the null hypothesis cannot be rejected. Thus, the RMSEA indicates that our model is a good fit.
...
is over-identified). Moreover, fixing the first loading to 1
scales the loadings (and the variances) to the scale of the item being
fixed. It is not necessarily crucial in our case, because all questions are over
the same 7 item scale, but standardizing ensures more readability. In that sense, if we consider the Assortment factor, a 1 unit increase in `Im1` will increase `Im2` by 0.88.
...
latent variables. Those values, when squared, represent the item's variance
...
It is the latent factors that influence the observed items, not the other
way around.

The function `modindices` quantifies the improvement of fit in our model if we would allow certain elements to be free instead of fixed.
...
The values of the modification indices, under column `mi`, aren't very high, which is reassuring. We try adding `Im21 ~~ Im22` to the model to see if that brings improvement. If Images 21 and 22 have a big modification index, it means that they share variance which is unexplained through their construct.
...
```
The model we obtain is very similar to our previous one, with ony the loadings of the Shopping Experience construct which change. Overall, there is no significative improvement in the measures of fit. Thus, we
